  ...
  async heal(
    ...
    try {
      ...
      if (originalTestId) {
        ...
        const candidates = await page.evaluate((originalId: string) => {
          const allElements = document.querySelectorAll('[data-testid]');
          const results: Array<{ testId: string; similarity: number }> = [];
          ...
          for (const el of allElements) {
            ...
            // Levenshtein-like simple comparison
              ...
              const commonChars = [...normalizedOriginal].filter(c =>
                normalizedTest.includes(c)
              ).length;
      ...
      if (storedInfo) {
        const candidates = await page.evaluate((info: ElementInfo) => {
          const results: Array<{ testId: string; score: number }> = [];
