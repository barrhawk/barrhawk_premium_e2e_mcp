...
exports.idStrategy = exports.IdStrategy = void 0;
class IdStrategy {
    name = 'id';
    priority = 1;
    async heal(originalSelector, storedInfo, page) {
        ...
        try {
            ...
            const candidateIds = await page.evaluate((components) => {
                const allElements = document.querySelectorAll('[id]');
                const candidates = [];
                for (const el of allElements) {
                    const id = el.id;
                    let score = 0;
                    // Check how many components match
                    for (const comp of components) {
                        if (id.toLowerCase().includes(comp.toLowerCase())) {
                            score += 1;
                        }
                    }
...
exports.idStrategy = new IdStrategy();
