...
export class AriaStrategy implements IHealingStrategy {
  ...
  async heal(
    ...
    try {
      ...
      if (originalRole && originalAriaLabel) {
        const candidates = await page.evaluate(
          ({ role, label }: { role: string; label: string }) => {
            ...
            const results: Array<{ selector: string; similarity: number; label: string }> = [];
      ...
      if (originalAriaLabel) {
        const candidates = await page.evaluate((label: string) => {
          const elements = document.querySelectorAll('[aria-label]');
          const results: Array<{ label: string; similarity: number; tag: string }> = [];
          ...
          for (const el of elements) {
            ...
            if (normalizedElLabel === normalizedLabel) {
              ...
              const common = [...normalizedLabel].filter(c =>
                normalizedElLabel.includes(c)
              ).length;
      ...
      if (originalRole && storedInfo) {
        const candidates = await page.evaluate(
          ({ role, info }: { role: string; info: ElementInfo }) => {
            const elements = document.querySelectorAll(`[role="${role}"]`);
