...
export class TextStrategy implements IHealingStrategy {
  name = 'text' as const;
  priority = 4;
  ...
  async heal(
    ...
    // NEW: Extract hint from selector (e.g., #login-btn -> "login")
    if (!originalText) {
      const selectorMatch = originalSelector.match(/[#.]?([\w-]+)/);
      if (selectorMatch) {
        // Extract meaningful words from selector
        const hint = selectorMatch[1]
          .replace(/[-_]/g, ' ')
          .replace(/([a-z])([A-Z])/g, '$1 $2')
          .toLowerCase()
          .split(' ')
          .filter(w => w.length > 2 && !['btn', 'button', 'input', 'form', 'container', 'wrapper'].includes(w))
          .slice(0, 2)
          .join(' ');
        if (hint) {
          originalText = hint;
      ...
      const candidates = await page.evaluate(`
        (function(params) {
          ...
          for (var i = 0; i < elements.length; i++) {
            ...
            var elText = directText || fullText;
            if (!elText) continue;

            var normalizedEl = norm(elText);
            var similarity = 0;

            if (normalizedEl === normalized) {
              similarity = 1.0;
            } else if (norm(directText) === normalized) {
              similarity = 0.98;
            } else if (normalizedEl.indexOf(normalized) !== -1) {
              var ratio = normalized.length / normalizedEl.length;
              similarity = 0.7 + ratio * 0.25;
            } else if (normalized.indexOf(normalizedEl) !== -1 && normalizedEl.length > 5) {
              similarity = 0.6 + (normalizedEl.length / normalized.length) * 0.2;
            } else {
              var searchWords = normalized.split(/\\s+/);
              var elWords = normalizedEl.split(/\\s+/);
              var overlap = 0;
              for (var j = 0; j < searchWords.length; j++) {
                for (var k = 0; k < elWords.length; k++) {
                  if (elWords[k].indexOf(searchWords[j]) !== -1 || searchWords[j].indexOf(elWords[k]) !== -1) {
                    overlap++;
                    break;
                  }
                }
        ...
        })(${JSON.stringify({ text: originalText, tag: tagFilter, normalized: normalizedOriginal })})
      `);
...
export const textStrategy = new TextStrategy();
