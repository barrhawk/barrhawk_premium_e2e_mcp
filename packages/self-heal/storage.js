...
exports.SqliteStorage = exports.InMemoryStorage = void 0;
...
class InMemoryStorage {
    mappings = new Map();
    healingHistory = [];
    getMapping(originalSelector, url) {
    ...
    recordHealing(originalSelector, healedSelector, strategy, confidence, url, success) {
    ...
    invalidateMapping(originalSelector, url) {
        const key = this.makeKey(originalSelector, url);
...
class SqliteStorage {
    ...
    saveMapping(mapping) {
      ...
      DO UPDATE SET
        ...
        is_valid = 1
    `).run(id, mapping.originalSelector, mapping.healedSelector, urlPattern, mapping.strategy, mapping.confidence, JSON.stringify(mapping.elementInfo), now, now);
    }
    recordHealing(originalSelector, healedSelector, strategy, confidence, url, success, healingTimeMs) {
        this.db.prepare(`
      INSERT INTO healing_history
        (original_selector, healed_selector, strategy, confidence, url, success, healing_time_ms, timestamp)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(originalSelector, healedSelector || null, strategy || null, confidence, url, success ? 1 : 0, healingTimeMs || null, new Date().toISOString());
    }
    getStats() {
    ...
    `).all();
        ...
        for (const row of byStrategyRows) {
            if (row.strategy && byStrategy[row.strategy]) {
        ...
        return {
            ...
            byStrategy,
            recentHealings: recentRows.map((row) => ({
                originalSelector: row.original_selector,
                healedSelector: row.healed_selector,
                strategy: row.strategy,
                confidence: row.confidence,
 ...
 */
async function getStorage(dbPath) {
    if (storage)
        return storage;
    if (dbPath) {
