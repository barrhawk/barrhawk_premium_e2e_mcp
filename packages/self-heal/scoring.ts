 ...
 */
function calculateAttributeMatch(found: ElementInfo, stored: ElementInfo): number {
  let matches = 0;
  let total = 0;
  ...
  if (stored.tagName) {
    total += 1;
    if (found.tagName === stored.tagName) matches += 1;
  }

  // ID match
  if (stored.id) {
    total += 2; // ID is important
    if (found.id === stored.id) matches += 2;
    else if (found.id && found.id.includes(stored.id)) matches += 1;
  }

  // data-testid match
  if (stored.testId) {
    total += 2;
    if (found.testId === stored.testId) matches += 2;
    else if (found.testId && found.testId.includes(stored.testId)) matches += 1;
  }

  // aria-label match
  if (stored.ariaLabel) {
    total += 1.5;
    if (found.ariaLabel === stored.ariaLabel) matches += 1.5;
    else if (found.ariaLabel && found.ariaLabel.includes(stored.ariaLabel)) matches += 0.75;
  }

  // Type match (important for inputs)
  if (stored.type) {
    total += 1;
    if (found.type === stored.type) matches += 1;
  }

  // Name match
  if (stored.name) {
    total += 1;
    if (found.name === stored.name) matches += 1;
  }

  // Placeholder match
  if (stored.placeholder) {
    total += 0.5;
    if (found.placeholder === stored.placeholder) matches += 0.5;
  }
 ...
 */
function calculateTextSimilarity(found: ElementInfo, stored: ElementInfo): number {
 ...
 */
function calculateStructuralMatch(found: ElementInfo, stored: ElementInfo): number {
  let score = 0;
  let factors = 0;
  ...
  if (stored.parent && found.parent) {
    ...
    if (stored.parent.tagName === found.parent.tagName) {
      ...
      if (stored.parent.classes && found.parent.classes) {
        const overlap = stored.parent.classes.filter(c =>
          found.parent!.classes!.includes(c)
        ).length;
        if (overlap > 0) {
 ...
 */
function calculateWeightedScore(factors: ScoreFactors, weights: ScoringWeights): number {
 ...
 */
function generateExplanation(factors: ScoreFactors, strategy: string): string {
  const parts: string[] = [];
...
export function rankCandidates(
  ...
  storedInfo: ElementInfo | undefined,
  weights: ScoringWeights = DEFAULT_WEIGHTS
): Array<{ result: StrategyResult; score: ScoreBreakdown }> {
 ...
 */
export function meetsThreshold(score: number, threshold: number = 0.7): boolean {
