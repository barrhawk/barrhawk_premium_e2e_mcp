...
export class SelfHealingManager {
  ...
  async heal(request: HealingRequest, page: any): Promise<HealingResult> {
    ...
    const healingPromise = (async () => {
      console.error(`[HEAL] Trying ${strategiesToTry.length} strategies for: ${originalSelector}`);
      for (const strategy of strategiesToTry) {
        console.error(`[HEAL] Running strategy: ${strategy.name}`);
        try {
          const result = await strategy.heal(originalSelector, storedInfo, page);
          console.error(`[HEAL] Strategy ${strategy.name} result: found=${result.found}, confidence=${result.confidence}, details=${result.details}`);
